/**
 * Tipos centralizados para o sistema de tabelas
 * 
 * @module table.types
 */
 ...
 */
export interface TableQueryParams<TFilters extends FilterMap = FilterMap> {
  /** Página atual (1-indexed) */
  page: number;
  /** Tamanho da página */
  page_size: number;
  ...
  [key: string]: FilterValue | number | undefined;
}
...
export interface DataTableProps<TData = unknown> {
  ...
  // Paginação
  /** Paginação externa (do hook) */
  pagination?: PaginationResult & { totalRecords: number };
  /** Opções de itens por página */
  pageSizeOptions?: number[];
  
  // Ordenação
  /** Ordenação externa (do hook) */
  sorting?: SortingResult;
  ...
  /** Callback de seleção */
  onSelectionChange?: (selection: TData | TData[]) => void;
  /** Modo de seleção */
